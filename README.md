# Khawaja Saad Akbar
**Full-Stack Engineer · Lahore, Pakistan**

---

> *I've had the rare privilege of working inside both worlds, legacy Rails monoliths that quietly power millions of transactions, and modern microservice architectures chasing the next abstraction.*
>
> *It teaches you humility. The old systems aren't broken — they're just misunderstood. And the new ones aren't solutions, they're bets.*
>
> *I try to find my place in this orchestration of technological change. Not chasing hype, not dismissing it either. Just building things that hold, whether they're five years old or five days old.*

---

## Stack

`Ruby on Rails` `Go` `FastAPI` `React / Next.js` `Node.js` `PostgreSQL` `Redis` `RabbitMQ` `Docker` `AWS` `GPT-4o / LLMs`

---

## Things I've Built & Contributed To

**[Manaiger](https://github.com/Xenax33)**  
A contract risk analysis SaaS — LLM-powered clause extraction, async background processing, PDF report generation. Built end-to-end, solo.  
`Rails` · `GPT-4o` · `Sidekiq` · `AWS`

**Crystal Commerce**  
Marketplace sync microservice in Go via RabbitMQ. Activity log consolidation into TimescaleDB. Cut database infrastructure costs by 20%.  
`Go` · `Next.js` · `FastAPI` · `TimescaleDB` · `RabbitMQ`

**Groovepacker**  
Eliminated N+1 queries, introduced Redis caching, built Scan to Cart end-to-end across a multi-tenant warehouse management platform.  
`Rails` · `React Native` · `MySQL` · `Redis`

---

## Education

🎓 **University of Engineering and Technology, Lahore (UET)**  
BS Computer Science · CGPA 3.4 / 4.0 · Teaching Assistant, CS Department

---

## Reach Me

📧 saadkbr8@gmail.com  
🔗 [linkedin.com/in/khawaja-saad-akbar](https://www.linkedin.com/in/khawaja-saad-akbar/)
